...
int wscore(string s)
{
    ...
    {
        l = toupper(s[i]);
        if(l == 'A' || l == 'E' || l == 'I' || l == 'O' || l == 'R' || l == 'S' || l == 'T')
            score++;
           
        else if (l == 'B' || l == 'C' || l == 'F' || l == 'M' || l == 'P' || l == 'W')
            score += 4;
            
        else if (l == 'D' || l == 'L' || l == 'N' || l == 'U')
            score += 2;
...
bool lookup(string s)
{
    ...
    {
        mdpt = (min + max)/2;
        if (strcmp(s, dictionary.words[mdpt].letters) < 0)
            max = mdpt;
        else if (strcmp(s, dictionary.words[mdpt].letters) > 0)
            min = mdpt;
            
        else                    
            if (!dictionary.words[mdpt].found)
            {
                dictionary.words[mdpt].found = true;
                return true;   
            }
            else
                return false; 
...
{
    //char temp;
    int tempg[DIMENSION][DIMENSION];
    for(int i = 0;i < DIMENSION; i++)
    {
        for(int j = 0;j < DIMENSION; j++)
        {
            tempg[i][j] = grid[DIMENSION - 1-j][i];     
...
void inspiration()
{
    bool w3 = false, w4 = false, w5 = false;
    for(int i = 0;i < dictionary.size; i++)
    {
        string gword = dictionary.words[i].letters;
        //printf("%s\n", gword);
        //printf("check");
        if(find(gword) && !dictionary.words[i].found)
        {
